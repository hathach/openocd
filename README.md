...
| [MounRiver Studio](http://www.mounriver.com/download) toolchain | `tcl/target/wch-riscv.cfg`, `wch-arm.cfg`, `wch-dual-core.cfg` and `ch32f1x.cfg`, taken from its OpenOCD (`bin/` and `share/openocd/scripts/target/`) so the WCH configs ship with the build instead of living in each project. Only the `[find]` paths were made search-path relative and CRLF converted to LF. |
