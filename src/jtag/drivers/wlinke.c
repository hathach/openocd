...
int wlink_init(void)
{
	...
	/* also anchors sdi.c into the link so its transport registers */
	if (transport_is_sdi())
		LOG_DEBUG("sdi transport selected (vendor-config compatibility)");
	if(iocontrol){
			switch (iocontrol)
				{
				case 1:
...
/*
 * Probe-side chip resets. The QingKe debug module does not survive a
 * standard ndmreset over the single-wire link (the session never comes
 * back), so resets must go through the WCH-Link protocol, which
 * re-attaches the debug module afterwards.
 */
COMMAND_HANDLER(wlink_handle_reset_run)
{
	wlink_quitreset(); /* reset chip, let it run */
	return ERROR_OK;
}

COMMAND_HANDLER(wlink_handle_reset_halt)
{
	wlink_reset(); /* reset chip, hold it halted under debug */
	return ERROR_OK;
}
...
static const struct command_registration wlink_command_handlers[] = {
	...
	{
		.name = "wlink_reset_run",
		.handler = &wlink_handle_reset_run,
		.mode = COMMAND_EXEC,
		.help = "reset the chip via the WCH-Link probe and let it run",
		.usage = "",
	},
	{
		.name = "wlink_reset_halt",
		.handler = &wlink_handle_reset_halt,
		.mode = COMMAND_EXEC,
		.help = "reset the chip via the WCH-Link probe and hold it halted",
		.usage = "",
	},
	{
		.name = "rst_set",
 ...
 * JTAG DTM emulation.
 *
 * The WCH-Link protocol exposes the RISC-V Debug Module directly: DMI_OP()
 * is one synchronous DMI transfer (probe handles run/idle and completion).
 * Instead of porting the fork's sdi transport and wch_riscv target stack,
 * present a standard JTAG RISC-V DTM (IDCODE/DTMCS/DMI) to mainline's
 * riscv target and translate DMI register scans into DMI_OP() calls.
 */

#define WLINK_IR_IDCODE 0x01
#define WLINK_IR_DTMCS  0x10
#define WLINK_IR_DMI    0x11
#define WLINK_IR_BYPASS 0x1f

/* QingKe taps on the rig are declared -irlen 5 -expected-id 0x00001 */
#define WLINK_IDCODE 0x00000001
/* dtmcs: abits=8 [9:4], version=1 (spec 0.13/1.0) [3:0], idle=0 */
#define WLINK_DTMCS  ((8 << 4) | 1)
#define WLINK_ABITS  8

#define DTM_DMI_OP_BITS   2
#define DTM_DMI_DATA_BITS 32

static uint32_t wlink_ir = WLINK_IR_IDCODE;
/* response to the previous DMI request, captured by the next DMI scan */
static uint8_t wlink_dmi_prev_op;
static uint32_t wlink_dmi_prev_data;
static uint8_t wlink_dmi_prev_addr;

static void wlink_emu_scan_field(const struct scan_command *scan)
{
	/* concatenate all fields of this scan into one bit string */
	unsigned int total_bits = jtag_scan_size(scan);
	uint8_t *out = calloc(1, DIV_ROUND_UP(total_bits, 8));
	uint8_t *in = malloc(DIV_ROUND_UP(total_bits, 8));
	unsigned int bit = 0;

	/* Bits past our (only) tap's response read back as ones, like TDI
	 * pulled high past the end of a real chain - jtag_examine_chain
	 * relies on that to detect the end of the chain. */
	memset(in, 0xff, DIV_ROUND_UP(total_bits, 8));

	for (unsigned int i = 0; i < scan->num_fields; i++) {
		const struct scan_field *f = &scan->fields[i];
		if (f->out_value)
			buf_set_buf(f->out_value, 0, out, bit, f->num_bits);
		bit += f->num_bits;
	}

	if (scan->ir_scan) {
		wlink_ir = buf_get_u32(out, 0, MIN(total_bits, 32));
		/* capture-IR must read b01 in the two LSBs (IEEE 1149.1);
		 * the remaining bits keep the ones fill so end-of-chain
		 * validation sees TDI passthrough */
		buf_set_u32(in, 0, MIN(total_bits, 2), 0x01);
	} else {
		switch (wlink_ir) {
		case WLINK_IR_IDCODE:
			buf_set_u32(in, 0, MIN(total_bits, 32), WLINK_IDCODE);
			break;
		case WLINK_IR_DTMCS:
			buf_set_u32(in, 0, MIN(total_bits, 32), WLINK_DTMCS);
			/* dmireset/dmihardreset writes need no action: DMI_OP is
			 * synchronous and never leaves an operation in flight */
			break;
		case WLINK_IR_DMI: {
			uint8_t req_op = buf_get_u32(out, 0, DTM_DMI_OP_BITS);
			uint32_t req_data = buf_get_u32(out, DTM_DMI_OP_BITS,
					DTM_DMI_DATA_BITS);
			uint8_t req_addr = buf_get_u32(out,
					DTM_DMI_OP_BITS + DTM_DMI_DATA_BITS,
					total_bits - DTM_DMI_OP_BITS - DTM_DMI_DATA_BITS);

			/* All WCH parts are single-hart, but some (CH32V103, CH58x)
			 * echo the hartsel bits back as writable instead of keeping
			 * them zero, which sends the riscv examine into a 2^20-hart
			 * enumeration. Present a conformant single-hart DM: keep
			 * hasel/hartsello/hartselhi zero in dmcontrol writes (the
			 * chip ignores them; the vendor target never set them). */
			if (req_op == 2 && req_addr == 0x10)
				req_data &= ~0x07FFFFC0u;

			/* shift out the previous request's response */
			buf_set_u32(in, 0, DTM_DMI_OP_BITS, wlink_dmi_prev_op);
			buf_set_u32(in, DTM_DMI_OP_BITS, DTM_DMI_DATA_BITS,
					wlink_dmi_prev_data);
			buf_set_u32(in, DTM_DMI_OP_BITS + DTM_DMI_DATA_BITS,
					total_bits - DTM_DMI_OP_BITS - DTM_DMI_DATA_BITS,
					wlink_dmi_prev_addr);

			/* Execute the new request (op 0 = nop: keep last response).
			 * A busy status must be absorbed here: this emulation is
			 * synchronous, so the riscv target's usual recover-by-nop-scan
			 * would only ever re-read the stale busy result. Mirror the WCH
			 * fork's own loop: clear abstractcs.cmderr and retry the op. */
			if (req_op != 0) {
				unsigned char o_addr = 0, o_op = 0;
				unsigned long o_data = 0;
				int64_t deadline = timeval_ms() + 2000;
				for (;;) {
					if (!DMI_OP(gIndex, req_addr, req_data, req_op,
							&o_addr, &o_data, &o_op)) {
						LOG_ERROR("wlinke: DMI transfer failed (addr=0x%x op=%u)",
								req_addr, req_op);
						o_op = 2; /* DMI_STATUS_FAILED */
						o_data = 0;
						o_addr = req_addr;
						break;
					}
					if (o_op != 3) /* not busy: done (0=ok, 2=failed) */
						break;
					/* busy: clear abstractcs.cmderr (0x16, W1C bits 10:8)
					 * like the WCH fork, then retry the operation */
					unsigned char c_addr = 0, c_op = 0;
					unsigned long c_data = 0;
					DMI_OP(gIndex, 0x16, 0x700, 2, &c_addr, &c_data, &c_op);
					if (timeval_ms() > deadline) {
						LOG_ERROR("wlinke: DMI stayed busy (addr=0x%x op=%u)",
								req_addr, req_op);
						o_op = 2;
						o_data = 0;
						o_addr = req_addr;
				...
				wlink_dmi_prev_op = o_op;
				wlink_dmi_prev_data = (uint32_t)o_data;
				wlink_dmi_prev_addr = o_addr;
			}
			break;
		}
		default: /* BYPASS and anything unknown: all-zero capture */
			break;
		}
	}

	/* scatter the captured bits back into the requesting fields */
	bit = 0;
	for (unsigned int i = 0; i < scan->num_fields; i++) {
		const struct scan_field *f = &scan->fields[i];
		if (f->in_value)
			buf_set_buf(in, bit, f->in_value, 0, f->num_bits);
		bit += f->num_bits;
	}
...
static int wlink_execute_queue(struct jtag_command *cmd_queue)
{
	for (struct jtag_command *cmd = cmd_queue; cmd; cmd = cmd->next) {
		switch (cmd->type) {
		case JTAG_SCAN:
			wlink_emu_scan_field(cmd->cmd.scan);
			break;
		case JTAG_TLR_RESET:
			wlink_ir = WLINK_IR_IDCODE;
			break;
		case JTAG_SLEEP:
		...
		case JTAG_TMS:
			/* no wire to wiggle: DMI_OP is a synchronous USB round trip */
			break;
		default:
			LOG_ERROR("wlinke: unknown JTAG command type %d", cmd->type);
			return ERROR_FAIL;
		}
	}
	return ERROR_OK;
...
static struct jtag_interface wlink_jtag_interface = {
	.execute_queue = wlink_execute_queue,
};

struct adapter_driver wlinke_adapter_driver = {
	.name = "wlinke",
	/* sdi is accepted for vendor-fork config compatibility; both
	 * transports drive the same JTAG DTM bridge above */
	.transport_ids = TRANSPORT_JTAG | TRANSPORT_SDI,
